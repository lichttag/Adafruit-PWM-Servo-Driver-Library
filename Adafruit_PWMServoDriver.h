...
class Adafruit_PWMServoDriver {
 public:
  ...
  Adafruit_PWMServoDriver(TwoWire *I2C);
  void begin(uint8_t i2cAddr);
  void reset(uint8_t i2cAddr);
  void setPWMFreq(uint8_t i2cAddr, float freq);
  void setPWM(uint8_t i2cAddr, uint8_t num, uint16_t on, uint16_t off);
  void setPin(uint8_t i2cAddr, uint8_t num, uint16_t val, bool invert=false);

 private:
  TwoWire *_i2c;
  bool _twiIsInitialized;
